...
### 變更
- **`refresh_shopee_session` 遇到登入頁時繼續等人。** 等待迴圈原本只在網址含
  `verify` 時等；2026-09-22 蝦皮把視窗導向登入頁，迴圈一輪就當成「已通過」，
  兩秒後關掉視窗，使用者還看著登入表單。現在網址含 `login` 也算牆。
  提示文字同步改成「滑動驗證，必要時先登入」。
...
- **筆電｜桌機切換移到側邊欄最上方，只放圖示。** 主畫面第一筆物件之前少了一層。
  側邊欄收起時（包括手機預設狀態）切換改顯示在標題下方，不會因為側邊欄收起就找不到桌機
  （見 [decisions #44](docs/decisions.md#44-第一筆物件之前只留必要的層切換移進側邊欄說明收進清單表頭)）。
- **「VFM 分數構成」收進清單表頭的「分數怎麼算？」。** 少了一整條可展開區塊，
  展開和收合不會重跑頁面。
- **手機版統計列排成一行**，第一筆物件上移約一排卡片的高度。
...
- **`docs/decisions.md` 加上依主題分組的目錄**（32 則分為爬蟲、解析、資料庫語意、
  ...
  等於沒寫。
- **`check_docs.py` 驗證標題錨點**。指向不存在標題的連結在瀏覽器裡不會報錯——
  它捲到頁首，看起來像跳轉比較慢，所以長文件的目錄會爛得無聲無息。
  slug 規則比對過 GitHub 實際渲染的 32 個 id，破折號、全形逗號、括號的邊界都涵蓋。
...
### 修正
- **Gemini 請求加上逾時**（預設 30 秒，`GEMINI_TIMEOUT_SECONDS`）。SDK 預設無限等待；
  2026-09-22 模型過載（503）時一個請求不回應，本機排程卡到一小時上限被砍，
  評分、警報與 heartbeat 都沒執行。逾時的那筆本次跳過，下次排程再解析。
- **PTT 不再漏抓 Mac 販售文。** 原本清單來自 Atom feed，固定只有最新 20 篇；iPhone 18
  上市洗版時 20 篇只涵蓋 2.5 小時，而 pipeline 一天跑一次。2026-09-21 查核：過去 5 天
  11 篇符合條件的 Mac 販售文（含多台 M5）全部沒進資料庫，排程卻連續 6 晚顯示成功。
  現在改從看板列表頁往前翻，收錄最近 `PTT_LOOKBACK_HOURS`（預設 36）小時的文章，
  不受發文量影響；移除 `feedparser` 相依
  （見 [decisions #46](docs/decisions.md#46-ptt-改讀看板列表頁範圍以時間決定不以篇數)）。
- **已刪除、已售出的物件不再掛在 Dashboard 上兩週。** 爬蟲只讀最新物件，一筆資料通常只被
  看到一次，之後沒有人再去看它。2026-09-21 查核：旋轉拍賣 86 筆在架物件中 22 筆已是
  404／410，PTT 17 筆中 5 筆 404。pipeline 現在會回查在架物件（PTT、旋轉拍賣），
  已刪除者直接下架、售出者標為售出；首次實跑另找出 3 筆賣家已改標題為售出的 PTT 文章。
  heartbeat 每個來源多一行回查結果。蝦皮不回查，理由見
  [decisions #45](docs/decisions.md#45-回查已存物件年齡老化降為後備蝦皮不回查)。
- **再次看到的物件會更新 `last_seen`。** 原本物件第二次出現在爬取結果裡時什麼都不做，
  所有在架資料都是 `first_seen = last_seen`，還在賣的物件也會在第 14 天被當成下架。
- **切換筆電／桌機時，圖例的「普通」「偏貴」會左右移動。** 原因是數字寬度不一，
  後面的項目跟著位移；現在每項佔固定寬度、數字等寬。
- **手機版圖例的「偏貴」會擠到第二行。**
- **切換的樣式一直沒有生效。** 原本的 CSS 選擇器 `stSegmentedControl` 在
  Streamlit 1.62 抓不到任何元素。
- **那筆 5,000 元的「M1」mac mini 清掉隔晚又長回來，這次連產生它的路徑一起堵住**。
  `decisions #40` 已經讓抽取規則拒絕這個標題，但 Step 2 只在 LLM 沒給晶片時才問
  抽取器——LLM 從同一個標題讀出 `M1`，於是它沒被問到；而年份推論又把 Mac mini + M1
  改寫成 2020，「早於 2020 就丟棄」也失效。時間欄位留下了證據：`first_seen` 9/12
  （#40 當天清掉），`updated_at` 9/13 02:41（隔晚被解析回來）。
  兩條規則現在抽成 `is_intel_era_title()`，可獨立於晶片來源被詢問；Step 2 的
  `_is_intel_era_row()` 同時問年份與標題；蝦皮 L1 加上同一道檢查，這種標題不再
  進得了資料庫。正式資料庫已重驗並清掉 2 筆
  （見 [decisions #43](docs/decisions.md#43-標題級的否決必須獨立於晶片從哪裡來)）。
- **蝦皮的規格選項現在會被當成標題的一部分檢查**。打平後的標題是 `商品名 - 規格名`，
  而店家真正在賣的機器寫在選項裡——標題掛「MacBook Pro M1」，選項卻是「2017 i5」。
  原本的過濾只看過商品名，等於只讀了標題沒讀商品。
- **旋轉拍賣補上 Intel 世代過濾**，三個爬蟲的第一層防線終於一致。它不會再算錯分數
  （Step 2 已擋住），補的是另外兩件事：進來的每一列都要花一次 LLM 呼叫才被丟掉，
  以及只有一個爬蟲沒檢查會讓下一個讀它的人以為那裡不需要檢查。
- **`revalidate_chips.py` 會呼叫 `configure_logging()`**。它原本在印出第一個含 emoji
  的賣家標題時就死在 cp950 主控台上——那正是它被寫出來要找的那種資料。

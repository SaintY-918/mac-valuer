...
**目標**：對外提供在售物件與其 CP 值查詢，並以 Streamlit Dashboard 呈現。

**不做估價。** 系統不接受「我有一台 X，值多少錢」這種輸入——它蒐集在售物件，
用每千元買到多少效能來排序。用詞必須守住這個界線：頁面標題曾經寫「二手 MacBook 智慧估價系統」，
宣稱了一個沒有實作的功能。
...
- **「筆電｜桌機」是模式，不是篩選。** 只放圖示的底線分頁，位於側邊欄最上方，預設筆電；
  側邊欄收起時改顯示在標題下方，兩者共用 `device_class`，畫面上只顯示一個（decisions #44）。
...
- **賣場範本的分級說明不算機況**：店家會在每一則商品底下貼同一段
  「● 70%新 = 商品瑕疵或功能異常」，那是定義不是描述。一台全新未拆的 MacBook Neo
  因此被標成瑕疵。凡是「兩到三位數%新」後面接 `=`／`：` 的整行，掃描前先移除；
  商品自己的「【新舊程度】全新品 / 100%新」沒有等號，保留。
- **規則改變後以 `src/scripts/revalidate_defects.py` 重驗全庫**（decisions #23）：
  瑕疵是解析時一次算好存進 `parsed_json` 的，規則改了已存的判定不會自己跟上。

...
- 每個 Gemini 請求都有逾時，預設 30 秒（`GEMINI_TIMEOUT_SECONDS`）。**SDK 預設是無限等待**，
  2026-09-22 模型過載時一個請求不回應，pipeline 卡到排程一小時上限被砍，評分、警報與
  heartbeat 都沒有執行。
- 逾時視同一般解析錯誤：該筆回傳 `None`、本次跳過，下次排程再解析；不重試，
  以免一個卡住的請求吃掉三倍時間。
- 上限的取法：`MAX_REPAIR_CALLS_PER_RUN`（50）次全部逾時仍須落在一小時內，所以不能設到 60 秒以上。

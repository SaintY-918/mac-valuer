...
- **過濾機制 (Gatekeepers)**：所有 `RawListing` 進入後續流程前需通過：
  1. **L1**: `5000 <= 價格 <= 150000`、標題不包含 `['殼', '膜', '零件機', '報廢']`、
     通過 `detect_product()`，且 `is_intel_era_title()` 為否。
     - 此處用的是「標題出現 Intel 世代訊號」而非 `mentions_apple_silicon()`：
       標題沒寫晶片的商品，描述裡可能有，詳情頁模式下 LLM 讀得到；標題寫了
       `i7/` 或 `2014` 則不是這種情況。
     - 少了這一條，`現貨mac mini m1 2012 2014/i7/...` 5,000 元仍會存進資料庫，
       被解析成 M1 而登上桌機榜首——抽取規則早就拒絕這個標題，只是沒有人在
       資料進來之前問它。
     - **變體標題需重新檢查**：詳情頁模式打平出的標題是 `商品名 - 規格名`，
       L1 只看過商品名。店家真正在賣的機器寫在**規格選項**裡（標題掛
       「MacBook Pro M1」，選項是「2017 i5」與「2020 M1」），因此組合後的
       標題必須再過一次 `is_intel_era_title()`，不通過者跳過該規格。
  2. **L2**: `stock > 0` (0 視為售出)。

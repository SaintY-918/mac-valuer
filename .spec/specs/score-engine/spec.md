...
- **年份可否決晶片**：Apple Silicon 自 2020 年 11 月起。解析出的年份早於 2020
  ...
  晶片只匹配標題裡的四個字元。
- **標題裡的 2006–2019 年份同樣否決晶片**（`PRE_SILICON_YEAR`），而且 `i5/i7` 後面接
  `/`、`,` 也算 Intel 標記。實測：`真猛電腦 現貨mac mini m1 2012 2014/i7/16g/500g ssd+ 4tb`
  是店家一則賣好幾代的「現貨」貼文，售價 5,000 是 2012 年那台的；`i7/` 沒被舊 regex 抓到，
  年份又被「M1 → 2020」的推論蓋掉，結果以 M1 之名、2012 之價拿到 1242 分、全站第一。
  **一個標題同時出現 Apple 晶片與 Intel 世代訊號，就不是一台機器**，不評分。
- **標題否決不可只靠年份欄位**。這兩條規則另外以 `is_intel_era_title(title)` 單獨提供，
  因為 `force_extract_chip()` 只在 LLM 沒給晶片時才被諮詢；LLM 從同一個標題讀出 `M1`
  時它根本不會被呼叫，而 `infer_correct_year()` 又會依桌機表把年份改寫成 2020，
  於是「年份早於 2020」的否決也失效。Step 2 的 `_is_intel_era_row(chip, year, title)`
  因此同時問年份與標題。爬蟲用同一個函式把這種物件擋在資料庫之外。
  邊界規則與 `CHIP_RE` 相同，`A2337` 這類機身型號與「保固至2025」不受影響。
  改動後以 `src/scripts/revalidate_chips.py` 重驗全庫（decisions #23）。
- MacBook Neo 在形態上歸入 `air13`（13 吋無風扇入門機），不落進 `pro13` 這個預設分支。

#### 6.X.4 螢幕尺寸顯示

Apple 行銷四種尺寸，賣家寫出七種：實測資料庫中有 13.0 / 13.3 / 13.6 / 14.0 / 14.2 /
15.0 / 15.6 / 16.2。實際對角線本來就會隨世代變動（Air 13 吋在 M1 是 13.3、M2 起是 13.6），
而 **15.6 吋 Apple 從未出過**，那是 Windows 筆電的規格。

- 顯示用 `nominal_inches()`，回傳 13 / 14 / 15 / 16 其中之一。
- **它以 `form_factor_key()` 的結果查表，不另外訂閾值**——卡片上的尺寸與計分用的
  形態加成因此不可能各說各話。
- 沒有解析到尺寸時回傳 `None`，卡片就不顯示尺寸。
  `form_factor_key()` 在缺值時預設 13.3 吋，那對「選一個乘數」是合理的保守值，
  但**印在卡片上等於宣稱一個沒人讀到的規格**。
- 原始值保留在 `parsed_json`，正規化只發生在顯示層。

...
反過來也成立：**真的看到了，就必須更新 `last_seen`。** pipeline Step 1 對「已在資料庫、
這次又出現在視窗裡」的物件曾經什麼都不做，於是 `last_seen` 永遠停在第一次目擊——
2026-09-21 查核時，所有 `available` 資料都是 `first_seen = last_seen`，
活著的物件一樣在第 14 天被 sweep 掉。再次目擊一律經由 `record_revisit()`
（已是 `sold` 者維持 `sold`）。

### `record_revisit(url, outcome)`：回查的結果

| outcome | `status` | `last_seen` |
|---|---|---|
| `available` | 設為 `available` | 更新（是目擊） |
| `sold` | 設為 `sold` | 更新（是目擊） |
| `gone` | 設為 `unavailable` | **不動**——頁面不存在，沒有看到任何東西 |

不用 `save_deal()`：那會以爬蟲結果覆寫標題、內文與狀態，而回查只有結論。
其他 outcome 一律 `ValueError`。
...
PTT 讀最近 `PTT_LOOKBACK_HOURS` 的文章，蝦皮讀最新約 180 筆搜尋結果。「這次沒看到」通常只代表它滾出了
視窗。曾經的集合式 sweep 在單一次執行中把 46 筆still-live 的蝦皮物件標成下架。

真正的下架判定留在該留的地方：PTT 的標題關鍵字、蝦皮的庫存與灰底判定——那些是**去看物件本身**。

但這些判定只在物件**還在視窗裡**時有機會執行。視窗只含最新物件，一筆資料通常只被看到一次，
之後賣家刪文、售出都不會被發現，只能等 sweep。2026-09-21 實測：旋轉拍賣 86 筆 `available`
中 22 筆已 404/410，PTT 17 筆中 5 筆 404。因此 pipeline 在 sweep 之前另有**回查**
（Step 1b，見 `scraper/spec.md` 3.U）：直接開物件頁，看到什麼就記什麼。
sweep 保留為後備，處理回查做不到的來源（蝦皮）與回查無法判斷的物件。
